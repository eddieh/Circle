...
@interface CircleFindFriendsTableViewController()
...
-(void) searchBar:(UISearchBar *) searchBar textDidChange:(NSString *)searchText
{
    if ([searchText length] != 0) {
        ...
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(name CONTAINS[c] %@) || (email CONTAINS[c] %@)",searchText,searchText];
        self.objects = [[self.origObjects filteredArrayUsingPredicate:predicate] mutableCopy];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    NSMutableArray *copyOfContacts = [NSMutableArray arrayWithArray:self.objects];
    ...
    if (currentString.length == 0) {
        self.filteredFriends = copyOfContacts;
    }
    [self.tableView reloadData];
    return YES;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    static NSString *CellIdentifier = @"userDescriptionCell";
    ...
    NSString *detailText;
    if ((detailText = [object objectForKey:@"email"]) != NULL) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //TODO: when friend is selected, add to array of stored frieds
}
